...
Hardcoded values (delivery fees, algorithm thresholds, feature toggles) were
scattered across the codebase, making configuration changes difficult and
error-prone. We need:
...
```typescript
// In OrdersService
const deliveryFee = await this.settingsService.getNumber(
  SETTINGS_KEYS.DELIVERY_FEE_IQD,
);

// In AnomalyDetectionService
const zThreshold = await this.settingsService.getNumber(
  SETTINGS_KEYS.ANOMALY_Z_THRESHOLD,
);
