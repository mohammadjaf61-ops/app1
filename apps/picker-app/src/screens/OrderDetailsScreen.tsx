...
import { UnavailableReason } from '@/lib/constants';
...
export function OrderDetailsScreen() {
  ...
  const handleCompleteOrder = () => {
    if (!allProcessed) {
      Alert.alert(
        'تنبيه',
        'يرجى تجهيز جميع المنتجات قبل إكمال الطلب',
        [{ text: 'حسناً' }]
      );
      return;
    }
            ...
            sortedItems.map((item) => {
              ...
              return (
                <PickItemCard
                  ...
                  status={status}
                  unavailableReason={
                    unavailableItems[item.id]?.reason || item.unavailableReason
                  }
                  ...
                  onUndo={
                    status !== 'pending' ? () => undoPickItem(item.id) : undefined
                  }
